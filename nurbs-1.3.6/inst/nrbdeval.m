...
%     [pnt, jac, hess, derhess] = nrbdeval (crv, dcrv, dcrv2, dcrv3, tt)
%     [pnt, jac, hess, derhess] = nrbdeval (srf, dsrf, dsrf2, dsrf3, {tu tv})
%
% INPUTS:
%
...
%    derhess - evaluated thrid derivatives
%
...
% third derivatives    
    if (nargout >= 4)
        if (exist ('dnurbs3'))
        % Ruuu
        [cuuup, cuuuw] = nrbeval (dnurbs3{1,1}, tt);
        tempuuu = cuuuw(ones(3,1),:,:);
        derhess{1,1} = (cuuup - (3.*cuup.*tempu + 3.*cup.*tempuu +...
            cp.*tempuuu)./temp + (6.*cup.*tempu.^2 + 6.*cp.*tempu.*tempuu)./temp.^2 - (6.*cp.*tempu.^3)./temp.^3)./temp;   
        
        % Rvvv
        [cvvvp, cvvvw] = nrbeval (dnurbs3{2,2}, tt);
        tempvvv = cvvvw(ones(3,1),:,:);
        derhess{2,2} = (cvvvp -(3.*cvvp.*tempv + 3.*cvp.*tempvv +...
            cp.*tempvvv)./temp + (6.*cvp.*tempv.^2 + 6.*cp.*tempv.*tempvv)./temp.^2 - (6.*cp.*tempv.^3)./temp.^3)./temp;

        % Ruuv
        [cuuvp, cuuvw] = nrbeval (dnurbs3{1,2}, tt);
        tempuuv = cuuvw(ones(3,1),:,:);
        derhess{1,2} = (cuuvp - (2.*cuvp.*tempu + cuup.*tempv + 2.*cup.*tempuv +...
            cvp.*tempuu + cp.*tempuuv)./temp + (2.*cvp.*tempu.^2 +...
            4.*cup.*tempu.*tempv + 4.*cp.*tempu.*tempuv + 2.*cp.*tempuu.*tempv)./temp.^2 -...
            (6.*cp.*tempu.^2.*tempv)./temp.^3)./temp; 
        
        % Ruvv
        [cvvup, cvvuw] = nrbeval (dnurbs3{2,1}, tt);
        tempvvu = cvvuw(ones(3,1),:,:);
        derhess{2,1} = (cvvup - (2.*cuvp.*tempv + cup.*tempvv + cvvp.*tempu +...
            2.*cvp.*tempuv + cp.*tempvvu)./temp + (2.*cup.*tempv.^2 +...
            4.*cvp.*tempu.*tempv + 4.*cp.*tempuv.*tempv + 2.*cp.*tempu.*tempvv)./temp.^2 -...
            (6.*cp.*tempu.*tempv.^2)./temp.^3 )./temp;  
        else
  ...
  if (nargout == 4 && exist ('dnurbs3'))
      ...
      derhess = (cuuup - (3*cuup.*temp1 + cup.*temp2 +cp.*temp3)./temp ...
          +(6*cup.*temp1.^2 + 6.*cp.*temp1.*temp2)./temp.^2 - 6*cp*temp1.^2./temp.^3)./temp;
  else
      warning ('nrbdeval: dnurbs3 missing. The third derivative is not computed');
      derhess = [];
  end
