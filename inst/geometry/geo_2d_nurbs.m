...
%           for ders = 3, the Hessian derivatives of the parametrization, evaluated at pts
%          
% 
...
function varargout = geo_2d_nurbs (nurbs, pts, ders)
  ...
  switch (ders)
    ...
    case 2
      ...
      % ------------ NEW PART ---------------------
      case 3
          [deriv, deriv2, deriv3] = nrbderiv (nurbs);
          %keyboard
          [F, jac, hessian, derhessian ] = nrbdeval (nurbs, deriv, deriv2, deriv3, pts);

      if (iscell (pts))
        npts = prod (cellfun (@numel, pts));
        derhess = zeros (2, 2, 2, npts);
        derhess(1:2, 1, 1, :) = reshape (derhessian{1,1}(1:2,:,:), 2, 1, 1, npts);
        derhess(1:2, 2, 2, :) = reshape (derhessian{2,2}(1:2,:,:), 2, 1, 1, npts);
        derhess(1:2, 1, 2, :) = reshape (derhessian{1,2}(1:2,:,:), 2, 1, 1, npts);
        derhess(1:2, 2, 1, :) = reshape (derhessian{2,1}(1:2,:,:), 2, 1, 1, npts);
      else
        derhess = zeros (2, 2, 2, size (pts, 2));
        derhess(1:2, 1, 1, :) = reshape (derhessian{1,1}(1:2,:), 2, 1, 1, size (pts, 2));
        derhess(1:2, 2, 2, :) = reshape (derhessian{2,2}(1:2,:), 2, 1, 1, size (pts, 2));
        derhess(1:2, 1, 2, :) = reshape (derhessian{1,2}(1:2,:), 2, 1, 1, size (pts, 2));
        derhess(1:2, 2, 1, :) = reshape (derhessian{2,1}(1:2,:), 2, 1, 1, size (pts, 2));
      end
      varargout{1} = derhess;
      % ----------------------------------------------
      otherwise
      error ('geo_2d_nurbs: number of derivatives limited to three')
  end
