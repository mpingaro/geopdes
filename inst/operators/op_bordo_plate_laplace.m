function mat = op_bordo_plate_laplace (spu, spv, msh, pt)
...
%                    sum (msh.jacdet(:,iel) .* msh.quad_weights(:, iel) .* sum( ish3(3,:) .* jshp', 1).');
       ...
       mat(spv.connectivity(:, iel), spu.connectivity(:, iel)) = ...
           mat(spv.connectivity(:, iel), spu.connectivity(:, iel)) +...
           (pt-1).*mat_loc_mn + (1-pt).*mat_loc_mnt + 2.*mat_loc_v_1 + (3-pt).*mat_loc_v_2;
   else
       warning ('geopdes:jacdet_zero_at_quad_node',...
           'op_bordo_plate_laplace: singular map in element number %d', iel)
   end
end
