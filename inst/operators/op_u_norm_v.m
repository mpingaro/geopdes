...
function varargout = op_u_norm_v (spu, spv, msh, coeff)
  
  %shpu = reshape (spu.shape_functions, spu.ncomp, msh.nqn, spu.nsh_max, msh.nel);
  
  shpu = squeeze (spu.deriv_normal(1,:,:,:));
  shpu = reshape (shpu, spu.ncomp, msh.nqn, spu.nsh_max, msh.nel);
  
  shpv = squeeze (spv.deriv_normal(1,:,:,:));
  shpv = reshape (shpv, spv.ncomp, msh.nqn, spv.nsh_max, msh.nel);
