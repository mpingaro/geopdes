...
% Trapezoidal mesh
% p11 =[0 0]; p12 =[base 0]; p22 =[2*base 2*altezza]; p21 =[0 altezza];
% srf_i = nrb4surf(p11,p12,p21,p22);
...
problem_data.bound_sides    = [3, 4];     % Define free edges (Boundary integrals)
...
%% Soluzione analitica:
x = 0:0.001:base;
% incastro incastro-rotazione
%sol = (pi/4/base).*x.^3-(pi/4).*x.^2;
%s = 2*base/3;
%min_def = (pi/4/base)*s^3-(pi/4)*s^2;
%disp(min_def);
% incastro -incastro-spostamento-rotazione
sol = ((-pi/4*base-2)/(base^3))*x.^3 + ((pi/4*base+3)/(base^2)).*x.^2;
s = 2/3*(3+pi/4*base)/(pi/4+2)*base;
max_spost = ((-pi/4*base-2)/(base^3))*s^3 + ((pi/4*base+3)/(base^2))*s^2;
disp(max_spost);

[eu_calc, F_calc] = sp_eval (u, space, geometry, {s,0});
disp(eu_calc);
