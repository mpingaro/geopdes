% OP_FRACTURE_RADI_TP: assemble the stiffness matrix 
% A = [a(i,j)], a(i,j) = (ls^2 laplace u_j, laplace v_i) + (grad u_j,grad v_i) , exploiting the tensor product structure.
%
%   mat = op_fracture_radi_tp (spu, spv, msh, epsilon);
%   [rows, cols, values] = op_fracture_radi_tp (spu, spv, msh, epsilon);
%
% INPUT:
%
...
% 
function varargout = op_fracture_radi_tp (space1, space2, msh, coeff1, coeff2, coeff3)
  Ag = spalloc (space2.ndof, space1.ndof, 3*space1.ndof);
  Agg = spalloc (space2.ndof, space1.ndof, 3*space1.ndof);
  ...
  for iel = 1:msh.nel_dir(1)
    ...
    Agg = Agg + op_gradgradu_gradgradv (sp1_col, sp2_col , msh_col, coeff2(x{:}));
    Al = Al + op_laplaceu_laplacev (sp1_col, sp2_col , msh_col, coeff3(x{:}));
