...
for iside = nmnn_sides
  ...
  rhs(sp_side.dofs) = rhs(sp_side.dofs) + op_f_v (sp_side, msh_side, gval);
   ...
   if iside == 1
      ...
      %qn{1} = zeros(1,size(qn{1},2)); % aggiunta
   ...
   elseif iside == 2
      ...
      rule_side{(1)} = [1; 2]; % [1, 2]
      [qn, qw] = msh_set_quad_nodes (zeta, rule_side);
      %qn{1} = ones(1,size(qn{1},2)); % aggiunto
...
%       normal.deriv(2,2,:,:)  = reshape (dnorm_y_y (x, y, iside), msh_side.nqn, msh_side.nel);
      ...
      rule_side{(2)} = [1; 2]; % [1, 2]
      [qn, qw] = msh_set_quad_nodes (zeta, rule_side);
      %qn{2} = ones(1,size(qn{2},2)); % aggiunto
      ...
      % Normal and Tangent derivatives
      sp_normal = sp_normal_tang(sp_side_row, msh_row, 'gradient', true,'hessian', true, 'der3', false);
...
drchlt_dofs2 = drchlt_dofs2(1) ;
u_drchlt2 = u_drchlt2(1) ;
...
%
[r_drchlt, drchlt_dofs_r] = sp_drchlt_l2_projj (space, msh, hr, drchlt_sides_r);
